...
CREATE OR REPLACE FUNCTION private.is_story_excluded(_story_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.is_story_excluded(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.is_story_excluded(uuid, uuid) TO authenticated, service_role;

CREATE OR REPLACE FUNCTION private.owns_story(_story_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION private.owns_story(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION private.owns_story(uuid, uuid) TO authenticated, service_role;
...
USING (
  ...
  OR (
    ...
    AND NOT private.is_story_excluded(id, auth.uid())
  )
);
...
FOR SELECT
TO authenticated
USING (private.owns_story(story_id, auth.uid()));
...
FOR INSERT
TO authenticated
WITH CHECK (private.owns_story(story_id, auth.uid()));
...
USING (
  user_id = auth.uid()
  OR private.owns_story(story_id, auth.uid())
);

DROP FUNCTION public.is_story_excluded(uuid, uuid);
DROP FUNCTION public.owns_story(uuid, uuid);